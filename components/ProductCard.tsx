...
export const ProductCard = ({
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    addToCart({ id, name, category, price, originalPrice, rating, reviews, image, badge });
    setAddedToCart(true);
  ...
  return (
    <div className="group bg-card rounded-2xl shadow-card hover:shadow-hover transition-all duration-500 overflow-hidden border border-border/50 hover:border-primary/30 hover-lift">
      <Link href={`/product/${id}`} className="block relative aspect-square bg-gradient-to-br from-secondary via-secondary/80 to-secondary/50 overflow-hidden">
        {/* Badges */}
        ...
        <button
          onClick={(e) => {
            ...
            if (isWishlisted) {
              ...
              addToWishlist({ id, name, category, price, originalPrice, rating, reviews, image, badge });
            }
          }}
        ...
        >
          <Heart className={`w-5 h-5 transition-all ${isWishlisted ? "fill-white scale-110" : ""}`} />
        </button>
